let handler = async (m, { conn, participants, groupMetadata, text }) => {
    ...
    try {
        ...
        let { isBanned, welcome, detect, sWelcome, sBye, sPromote, sDemote, antiLink, expired, antiToxic, delete} = global.db.data.chats[m.chat]
...
`.trim()
    ...
    conn.reply(m.chat, 'Maaf, info grup sedang error', m)
    throw e
      }
}
